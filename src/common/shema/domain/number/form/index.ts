...
/** Границе се задају уз поље, али се проверавају овде — уз остала правила истог домена. */
...
export const vForm = (decimals: number, maxIntegerDigits: number, bounds: Bounds = {}) => {
  const pattern = patternFor(decimals)

  const vanOpsega = (n: number): boolean =>
    (bounds.min !== undefined && n < bounds.min) || (bounds.max !== undefined && n > bounds.max)
      ...
      text === ''
        ...
        : !pattern.test(text)
          ...
          : integerDigits(text) > maxIntegerDigits
            ? 'previse cifara'
            : vanOpsega(round(Number(text), decimals))
              ? 'van opsega'
              : undefined,
    ),
  )
  ...
  return Schema.transform(digits, Schema.Number, {
    ...
    Annotation.message((value: Form) => {
      ...
      const broj = round(Number(text), decimals)
      if (bounds.min !== undefined && broj < bounds.min) return `Unesena vrednost ne sme biti manja od ${bounds.min}`
      if (bounds.max !== undefined && broj > bounds.max) return `Unesena vrednost ne sme biti veca od ${bounds.max}`
      return undefined
    }),
  )
}
